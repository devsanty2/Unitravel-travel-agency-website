...
if (!function_exists('unitravel_trx_addons_theme_setup9')) {
	...
	function unitravel_trx_addons_theme_setup9() {
		if (unitravel_exists_trx_addons()) {
			...
			add_filter( 'trx_addons_filter_override_options_fields',			'unitravel_trx_addons_override_options_fields', 10, 2);
			...
			if (is_admin()) {
				add_filter( 'unitravel_filter_allow_override_options', 			'unitravel_trx_addons_allow_override_options', 10, 2);
	...
	function unitravel_trx_addons_frontend_scripts() {
		if (unitravel_exists_trx_addons()) {
			if (unitravel_is_on(unitravel_get_theme_option('debug_mode')) && unitravel_get_file_dir('plugins/trx_addons/trx_addons.css')!='') {
				wp_enqueue_style( 'unitravel-trx-addons',  unitravel_get_file_url('plugins/trx_addons/trx_addons.css'), array(), null );
				wp_enqueue_style( 'unitravel-trx-addons-editor',  unitravel_get_file_url('plugins/trx_addons/trx_addons.editor.css'), array(), null );
			}
			if (unitravel_is_on(unitravel_get_theme_option('debug_mode')) && unitravel_get_file_dir('plugins/trx_addons/trx_addons.js')!='')
				wp_enqueue_script( 'unitravel-trx-addons', unitravel_get_file_url('plugins/trx_addons/trx_addons.js'), array('jquery'), null, true );
		} else {
...
if (!function_exists('unitravel_trx_addons_allow_override_options')) {
	
	function unitravel_trx_addons_allow_override_options($allow, $post_type) {
		return $allow
...
if (!function_exists('unitravel_trx_addons_override_options_fields')) {
	
	function unitravel_trx_addons_override_options_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
...
if ( ! function_exists( 'unitravel_trx_addons_privacy_text' ) ) {
	add_filter( 'trx_addons_filter_privacy_text', 'unitravel_trx_addons_privacy_text' );
	function unitravel_trx_addons_privacy_text( $text='' ) {
		return unitravel_get_privacy_text();
	}
}
...
.widget_instagram_follow_link_wrap a:hover,
